                    ...
                    <a href="/storyverses/${sv.id}" class="block p-4 bg-white dark:bg-[#161925]/40 border border-gray-200 dark:border-gray-800 hover:border-amber-500/40 rounded-xl hover:bg-gray-100 dark:bg-[#1a1e2e]/30 transition-all">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span class="text-xs text-gray-500 dark:text-gray-400 line-clamp-1 mt-0.5">${sv.description}</span>
                            </div>
                        </div>
                    </a>
                    `)}
        ...
        async function handleSaveSettings(e) {
            ...
            try {
                const res = await fetch('/api/settings', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ display_name: displayName, is_creator: isCreator, ai_author_name: aiAuthorName })
                });
                const data = await res.json();
                if (data.success) {
