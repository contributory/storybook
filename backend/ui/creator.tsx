...
export function renderCreatorPanel(books: db.Storybook[], universes: db.Storyverse[], prefillBookId = "") {
  return html`
    <div class="max-w-5xl mx-auto space-y-8 text-left">
        <div class="space-y-2">
            <h1 class="text-3xl font-black text-gray-900 dark:text-white"><i class="fa-solid fa-feather-pointed mr-2 text-amber-500"></i> Nhà Sáng Tạo</h1>
            <p class="text-gray-600 dark:text-gray-400">Tự do xây dựng tác phẩm, tóm tắt cốt truyện và xuất khẩu dữ liệu phục vụ sáng tác cùng AI.</p>
        </div>
            ...
            <div class="lg:col-span-2 bg-white dark:bg-[#161925]/30 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 sm:p-8 relative min-h-[450px]">
                    ...
                    <form onsubmit="handleCreateChapter(event)" class="space-y-4">
                        <div class="grid grid-cols-3 gap-4">
                            <div class="col-span-2">
                                <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-1.5">Chọn bộ truyện</label>
                                <select id="chapterBookId" required class="w-full bg-gray-50 dark:bg-[#0f111a] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-2.5 text-gray-700 dark:text-gray-300 focus:outline-none focus:border-amber-500 transition-colors">
                                    ${books.map(b => html`<option value="${b.id}" ${prefillBookId === b.id ? "selected" : ""}>${b.title}</option>`)}
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-1.5">Chương số</label>
                                <input type="number" id="chapterNumber" required min="1" class="w-full bg-gray-50 dark:bg-[#0f111a] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-2.5 text-gray-900 dark:text-white focus:outline-none focus:border-amber-500 transition-colors" placeholder="1">
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-1.5">Tiêu đề chương</label>
                            <input type="text" id="chapterTitle" required class="w-full bg-gray-50 dark:bg-[#0f111a] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-2.5 text-gray-900 dark:text-white focus:outline-none focus:border-amber-500 transition-colors" placeholder="Đại náo thiên cung">
                        </div>
                        <div>
                            <div class="flex items-center justify-between mb-1.5">
                                <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Tóm tắt chương truyện (Tối quan trọng cho AI)</label>
                                <span class="text-[10px] text-amber-500 font-medium">Giúp AI ghi nhớ cốt truyện nhanh mà không tốn token</span>
                            </div>
                            <textarea id="chapterSummary" required rows="2" class="w-full bg-gray-50 dark:bg-[#0f111a] border border-gray-200 dark:border-gray-800 rounded-xl p-3 text-gray-900 dark:text-white focus:outline-none focus:border-amber-500 transition-colors" placeholder="Ví dụ: Tôn Ngộ Không náo loạn điện Ngọc Đế, ăn trộm linh đơn của Thái Thượng Lão Quân, trốn về Hoa Quả Sơn..."></textarea>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-1.5">Nội dung chương</label>
                            <textarea id="chapterContent" required rows="8" class="w-full bg-gray-50 dark:bg-[#0f111a] border border-gray-200 dark:border-gray-800 rounded-xl p-3 text-gray-900 dark:text-white font-serif focus:outline-none focus:border-amber-500 transition-colors" placeholder="Nội dung chương truyện chính..."></textarea>
                        </div>
                ...
                <div id="tabContent-aiPromptExporter" class="creator-tab-content space-y-6 hidden">
                    ...
                    <div class="space-y-4">
                        ...
                        <div id="exportLoader" class="hidden text-sm text-gray-600 dark:text-gray-400 italic py-2">Đang tải và sinh ngữ cảnh...</div>
                        ...
                        <div id="exportResultContainer" class="hidden space-y-4 text-left">
                            ...
                            <span class="text-[10px] text-gray-500 dark:text-gray-500 leading-relaxed block">
    ...
    <script>
        ...
        function switchCreatorTab(tabName) {
            ...
            if (tabName === 'aiPromptExporter') {
                ...
                btn.className = 'creator-tab-btn w-full p-4 bg-white dark:bg-[#161925] border border-amber-500 text-amber-400 font-bold text-sm rounded-xl text-left flex items-center justify-between transition-all';
            }
